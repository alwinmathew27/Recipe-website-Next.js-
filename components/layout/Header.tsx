  ...
  return (
    <header className="sticky top-0 left-0 w-full bg-white shadow-md z-50 transition-all duration-300 ease-in-out">
      ...
      <div className={`hidden lg:block transition-all duration-300 ${
        isScrolled ? "pt-2 pb-1" : "pt-6 pb-2"
      }`}>
        <div className="flex items-center justify-between px-4 xl:px-8">
          {/* Logo */}
          <Link href="/" className="cursor-pointer flex-shrink-0">
          ...
          <div className="flex items-center mx-4">
            <input
              ...
              onChange={(e) => setSearchText(e.target.value)}
              placeholder="Find a recipe or ingredient"
              className={`border border-gray-400 placeholder:text-xs w-[250px] xl:w-[300px] rounded-l-md focus:outline-none transition-all duration-300  border-r-0 ${
                isScrolled ? "p-1 text-xs" : "p-2"
              }`}
            />
            <button
              onClick={handleSearch}
              className={`bg-[#D54215] border border-gray-400 rounded-r-md  border-l-0 cursor-pointer transition-all duration-300 ${
                isScrolled ? "p-1" : "p-2"
              }`}
            >
              <Image
          ...
          <div className="flex items-center gap-2 xl:gap-4 flex-shrink-0">
            ...
            {status === "loading" ? (
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <span className={`font-medium hidden xl:inline transition-all duration-300 ${
                ...
                <button
                  onClick={() => signOut()}
                  className={`text-red-500 underline cursor-pointer transition-all duration-300 ${
                    isScrolled ? "text-xs" : "text-sm"
                  }`}
                >
                  Logout
              ...
              <button
                onClick={() => signIn("github")}
                className={`flex items-center gap-1 px-2 text-black cursor-pointer transition-all duration-300 ${
                  isScrolled ? "text-xs" : "text-sm"
                }`}
              >
                <svg
        ...
        {/* Bottom Navigation */}
        <div className={`flex justify-between items-center px-4 xl:px-8 overflow-hidden transition-all duration-300 ${
